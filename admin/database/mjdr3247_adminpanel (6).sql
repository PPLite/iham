...
--
-- Host: 127.0.0.1
-- Generation Time: Jul 07, 2021 at 09:52 AM
...
--
-- Stand-in structure for view `tb_data_ruangan_siap`
-- (See below for the actual view)
--
CREATE TABLE `tb_data_ruangan_siap` (
`nama_pasien` varchar(50)
,`jenis_ruangan` varchar(100)
,`kelas_ruangan` varchar(100)
,`kategori_pasien` varchar(100)
,`kapasitas_ruangan` varchar(100)
,`harga` varchar(100)
);
...
--
-- Table structure for table `tb_hasil_survey`
--

CREATE TABLE `tb_hasil_survey` (
  `rfid_uid` int(11) NOT NULL,
...
CREATE TABLE `tb_kamar` (
  ...
  `kelas_ruangan` varchar(100) NOT NULL,
  `kategori_pasien` varchar(100) NOT NULL,
  `kapasitas_ruangan` varchar(100) NOT NULL,
...
--
-- Structure for view `tb_data_ruangan_siap`
--
DROP TABLE IF EXISTS `tb_data_ruangan_siap`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `tb_data_ruangan_siap`  AS SELECT `tb_pasien_dewasa`.`nama_pasien` AS `nama_pasien`, `tb_kamar`.`jenis_ruangan` AS `jenis_ruangan`, `tb_kamar`.`kelas_ruangan` AS `kelas_ruangan`, `tb_kamar`.`kategori_pasien` AS `kategori_pasien`, `tb_kamar`.`kapasitas_ruangan` AS `kapasitas_ruangan`, `tb_kamar`.`harga` AS `harga` FROM (`tb_pasien_dewasa` join `tb_kamar` on(`tb_pasien_dewasa`.`nama_pasien` = `tb_kamar`.`nama_pasien`)) ORDER BY `tb_pasien_dewasa`.`nama_pasien` ASC ;
